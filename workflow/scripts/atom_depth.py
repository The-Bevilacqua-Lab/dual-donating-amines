...
def atom_depth(pdb_id, model, eq_class_mem_id, donor_list, original_mmcif_dir, msms):
    parser = MMCIFParser()
    structure = parser.get_structure(pdb_id, f"{original_mmcif_dir}{pdb_id.lower()}.cif")
    surface = get_surface(structure[int(model)-1], MSMS=msms)
    distances = []
    for pymol_donor in donor_list:
        ...
        if pymol_donor[3][-1].isdigit():
            resseq = int(pymol_donor[3])
            icode = " "
        else:
            resseq = int(pymol_donor[3][:-1])
            icode = pymol_donor[3][-1]
            # This module does not currently handle insertion codes that are more than one character in length.
            if not pymol_donor[3][-2].isdigit():
        ...
        bio_pdb_donor = structure[int(model)-1][chain][(hetfield, resseq, icode)][name]
