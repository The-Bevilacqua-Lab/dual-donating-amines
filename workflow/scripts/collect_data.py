...
cmd.iterate(f'(resn TYR and name OH) within {search_dist} of ({rep_rna})',
            'cmd.unbond(f"index {index}", f"(byres index {index}) and name CZ"); '
            'cmd.bond(f"index {index}", f"(byres index {index}) and name CZ", "2")')
...
#           'formal_charge=1')
cmd.alter(f'(resn ARG and name NH2) (resn TYR and name OH) (resn HIS and name ND1)',
          'formal_charge=1')
...
cmd.iterate(f'(resn ARG and name NH2) within {search_dist + 3.0} of (({rep_rna}) and not elem H)',
            'stored.h_count.append(cmd.count_atoms(f"(neighbor index {index}) and elem H"))')
